document.addEventListener("DOMContentLoaded", async function () {
  ...
  `;
    ...
    dateCells.forEach((dateCell) => {
      ...
      } else if (!dateCell.classList.contains("has-entry")) {
        // If the date cell doesn't have an entry, add a click event listener to open the submission form.
        dateCell.addEventListener("click", function () {
          const date = dateCell.getAttribute("data-date");
    ...
    saveButton.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (result.ok) {
          ...
          // You may want to add some success message or close the pop-up here
        } else {
  ...
  function displayJournalPopup(selectedDate) {
    ...
    // Set the position of the pop-up container
    popupContainer.style.left = rect.left * 30 + "px";
    popupContainer.style.top = rect.top * 2.7 + "px";
